# 향료 원장 (Fragrance Registry)

각 콜렉션이 **어떤 향료를, 어느 회사에서, 어떤 코드로** 쓰는지 정리한 문서.
근거는 Google Drive에 보관된 IFRA 인증서와 알레르겐 선언서 원문.

조사일: 2026-09-09

---

# 1. 향료 매핑

**캔들과 바디는 향료 자체가 다르다.** 향료 회사까지 다른 경우도 있다.

| 콜렉션 | 라인 | 향료명 | 코드 | 향료사 | IFRA 개정판 | 인증서 발행일 |
|---|---|---|---|---|---|---|
| Bamboo Forest | **캔들** | **BAMBOO 28** | 28 | **DreamAir** | 확인 필요 | 확인 필요 |
| Bamboo Forest | **바디 · 바 솝** | **BAMBOO 34** (`BAMBOOM 34 LATEST`) | 6059239 | **IFF** (DreamAir 경유) | **50** | 2023-11-01 |
| ~~(단종) 소금~~ | ~~배스 솔트~~ | ~~078296 CORIANDER FRAGRANCE~~ | 78296 | Chemia | 50 (2021.6) | 2021-09-24 |
| Bushman Candle | **캔들 + 디퓨저** | **OLIFIR BOUGIE XII** | 26954 | AFM | **50** (2021.7) | 2023-06-07 |
| Bushman Candle | **바디 · 바 솝** | **OLIFIR V4i** | 26830 | AFM | **51** (2023.6) | 2024-01-25 |
| Shaman | **캔들** | **SPIRITUAL WOOD** | 28782 | AFM | **50** (2021.7) | 2023-04-21 |
| Valley of Roses | ? | **84033 ROSE BUD** | 84033 | Chemia | 확인 필요 | 확인 필요 |

- `BOUGIE`는 프랑스어로 **캔들**이다.

### 용도 확정 (`소영`, 2026-09-13)

| 향료 | 용도 |
|---|---|
| `BAMBOO 28` | **뱀부 포레스트 — 캔들** |
| `BAMBOO 34` (6059239) | **뱀부 포레스트 — 바디 · 바 솝** |
| `SPIRITUAL WOOD` (28782) | **샤먼 캔들** 향 |
| `OLIFIR BOUGIE XII` (26954) | **부시맨 캔들 — 캔들 + 디퓨저** |
| `OLIFIR V4i` (26830) | **부시맨 캔들 — 바디 · 바 솝** (미르 프로텍션 바) |

🔑 **뱀부 포레스트도 부시맨 캔들도 향료가 둘로 갈린다.**

| 콜렉션 | 캔들용 | 바디·솝용 |
|---|---|---|
| 뱀부 포레스트 | `BAMBOO 28` | `BAMBOO 34` |
| 부시맨 캔들 | `OLIFIR BOUGIE XII` | `OLIFIR V4i` |

태우는 것과 몸에 쓰는 것은 규제 체계가 달라 서류도 따로 관리한다.

🔑 **밸리 오브 로지즈만 하나의 향료(84033 Rose Bud)로 두 라인을 다 쓴다.**

⚠️ 디퓨저는 현재 솔드아웃 제외 대상이나, 향료 매핑 자체는 사실로 남긴다.
- AFM = Atelier Fragranze Milano (이탈리아). 송금 이력이 있는 그 회사다.
- Bamboo Forest는 **캔들과 바디의 향료 회사가 아예 다르다.**
  같은 콜렉션명이지만 서로 다른 향이다.

## 향료사 지도 (2026-09-13 확정, 소영님)

| 콜렉션 | 향료사 | 사람 |
|---|---|---|
| **뱀부 포레스트** | DreamAir / IFF | **Christophe Laudamiel** — DreamAir는 본인 회사. 원래 IFF 소속이었다 |
| **부시맨 캔들** · **샤먼** | **AFM** (Atelier Fragranze Milano, 이탈리아) | 대표 **Luca**, 조향 **Antoine Lie** |
| **밸리 오브 로지즈** | **Chemia** | **파운더 조소영이 Chemia와 함께 만들었다** |

🔑 **Chemia는 현행 제품 중 로즈 향 하나뿐이다.** 다른 콜렉션에는 관여하지 않는다.
   (과거 배스 솔트용 `78296 Coriander`도 Chemia였으나 단종됐다.)

🔑 **밸리 오브 로지즈만 파운더가 직접 조향에 참여했다.**
   AFM·DreamAir는 외부 조향사가 만들어 준 향이지만, 로즈는 함께 만든 향이다.
   카피에서 이 차이를 드러낼 수 있다.

🔑 **AFM의 Luca와 Antoine Lie는 같이 일한다.** 부시맨 캔들·샤먼 두 콜렉션의
   조향사가 Antoine Lie인 것과 향료사가 AFM인 것이 같은 관계에서 나온다.

---

## 🔴 2026-09-13 정정 (소영님)

| 확정 | 내용 |
|---|---|
| 뱀부 포레스트 조향 | **DreamAir** — 크리스토프 로다미엘 본인 회사. 원래 IFF 소속이었다가 독립 |
| **Chemia** | **로즈 향(84033 Rose Bud) 하나뿐이다** |
| 부시맨 캔들 · 샤먼 | **AFM** (Atelier Fragranze Milano, 이탈리아) — 기존 기록과 일치 |

✅ **`78296 CORIANDER FRAGRANCE`의 정체 확인 — 예전 소금 향료다.**

Chemia에서 만들었던 **구 소금(bath salt) 제품용 향료**이며,
현재 판매 중인 6종 어디에도 쓰이지 않는다 (`소영`, 2026-09-13).

제가 이것을 뱀부 포레스트 캔들 향료로 잘못 연결했었다.
코리앤더가 뱀부 포레스트의 향 노트 중 하나라서 이름이 겹쳤기 때문이다.

→ **현행 제품 매핑에서 제외한다.** 아카이브 기록으로만 남긴다.

✅ 뱀부 포레스트 캔들 향료는 **`BAMBOO 28`**로 확인됨 (`소영`, 2026-09-13).
   IFRA 인증서·알레르겐 선언서는 DreamAir에 요청 필요.

## 확인됨 (소영님)

- **Valley of Roses는 하나의 향으로 캔들과 바디 두 라인에 모두 사용한다.**
  Chemia 84033 Rose Bud 단일 향료. 다른 콜렉션처럼 캔들용/바디용이 나뉘지 않는다.

## 성분 표기 방식 — Fragrance / Parfum으로 통일

바 솝 2종의 향료 표기 방식이 서로 달라 통일이 필요하다.

| 제품 | 현재 표기 |
|---|---|
| Charcoal Detox Bar | `Fragrance/Parfum` 없이 라벤더·베르가못·패츌리 **에센셜 오일 개별 나열** |
| Myrrh Protection Bar | `Fragrance (parfum)` |

**결론: `Fragrance` (미국) / `Parfum` (EU·INCI) 표기가 가능하며, 이쪽으로 통일한다.**

- 조향사가 만든 향료 조성물이므로 `Parfum`이 정확한 표기다.
  에센셜 오일을 개별 나열하면 단일 오일만 쓴 것처럼 오해될 수 있다.
- ⚠️ **단, 기준치를 넘는 알레르기 유발성분은 `Parfum`으로 묶어도 별도 표기해야 한다.**
  - EU: 리브온 10 ppm / 린스오프 100 ppm 이상
  - 한국: 식약처 고시 25종, 리브온 0.001% / 린스오프 0.01% 초과
  - 미국: `Fragrance` 표기 허용 (21 CFR 701.3)
- 두 제품의 표기 방식을 반드시 동일하게 맞춘다.
  (Charcoal Bar 성분표에는 `Fragrance/Parfum` 없이 라벤더·베르가못·패츌리 **에센셜 오일**이
   직접 기재되어 있다. Myrrh Bar에는 `Fragrance (parfum)` 표기가 있다. 서로 방식이 다르다.)

---

# 2. Oakmoss — 바디 향료에만 있다

향료 노트와 배합은 **소영님이 조향사와 직접 논의해 만든 것**이며,
향료 서류도 소영님이 직접 관리한다. 향에 관한 사항은 소영님 확인이 원본이다.
아래는 그 내용을 서류로 뒷받침한 것이다.

오크모스는 캔들이 아니라 바디 향료에 들어 있다.

`BAMBOOM 34 LATEST` (Bamboo Forest **바디**) 알레르겐 선언서 (IFF, 2021-08-17, Doc ID 1306252)

| 알레르겐 | CAS | 향료 원액 내 함량 |
|---|---|---|
| **Evernia Prunastri (Oakmoss) Extract** | 68917-10-2 / 90028-68-5 | **1.218 %** |
| Limonene | 5989-27-5 | 4.015 % |
| Linalool | 78-70-6 | 1.747 % |
| Geraniol | 106-24-1 | 0.045 % |
| Citronellol | 106-22-9 | 0.003 % |
| Eugenol | 97-53-0 | 0.003 % |
| Citral | 5392-40-5 | 0.002 % |
| Benzyl Alcohol | 100-51-6 | 0.001 % |
| Farnesol | 4602-84-0 | 0.001 % |
| Evernia Furfuracea (Treemoss) Extract | — | 불검출 |

→ **캔들 향료(Bougie XII, Spiritual Wood, Coriander 78296) 문서에는 오크모스가 나오지 않는다.**
→ 사이트에서 캔들 설명에 오크모스가 등장한 것은 **바디 라인 정보가 섞여 들어간 것**이다.

### 표시 의무 계산 (검증 필요)

EU 기준 알레르겐 표시 의무: 리브온 10ppm 이상, 린스오프 100ppm 이상.

바디 제품(린스오프, IFRA Cat 9)에 최대 배합률 2.1%로 넣었다고 가정하면

| 알레르겐 | 완제품 농도 | ppm | 표시 의무 |
|---|---|---|---|
| Limonene | 0.0843 % | 843 | **표시 필요** |
| Linalool | 0.0367 % | 367 | **표시 필요** |
| **Oakmoss** | 0.0256 % | 256 | **표시 필요** |
| Geraniol | 0.00095 % | 9.5 | 불필요 |

⚠️ 위는 **최대 배합률 기준 추정**이다. **실제 배합률을 확인해야 확정된다.**
⚠️ Oakmoss(Evernia prunastri) 자체는 EU 화장품규정 Annex III 제한 물질이며,
   완제품 0.1% 상한이 있다. 위 추정치(0.0256%)는 상한 이내다.

---

# 3. 🔴 서류가 전부 구버전이다

## IFRA 개정판

| 향료 | 인증서 기준 | 문제 |
|---|---|---|
| OLIFIR BOUGIE XII (Bushman 캔들) | IFRA **50** (2021.7) | 구버전 |
| SPIRITUAL WOOD (Shaman 캔들) | IFRA **50** (2021.7) | 구버전 |
| Chemia 78296 (Bamboo Forest) | IFRA **50** (2021.6) | 구버전 |
| BAMBOOM 34 (Bamboo Forest 바디) | IFRA **50** | 구버전 |
| OLIFIR V4i (Bushman 바디) | IFRA **51** (2023.6) | 그나마 최신 |

→ **캔들 3종의 IFRA 인증서가 전부 IFRA 50 기준이다.** IFRA 51이 2023년에 나왔고
  이후 개정도 진행되었다. 리테일러 심사에서 지적받기 쉽다.
→ 각 향료사에 **최신 개정판 기준 인증서 재발급**을 요청해야 한다.

## 알레르겐 선언서

`BAMBOOM 34` 알레르겐 선언서는 **2021년 8월 17일자**이며,
**EU 화장품규정 개정 이전의 26종 알레르겐 목록** 기준이다.

→ Reg (EU) 2023/1545로 표시 대상 알레르겐이 대폭 확대되었다.
→ 2021년 서류로는 현행 EU 라벨을 만들 수 없다. **재발급 필수.**
→ 시행 시점과 유예기간은 **EU RP(책임자)에게 확인할 것.**
   메일함에 이탈리아 CPNP/RP 담당 **조경상**님 연락처가 있다 (tradeit.cho@gmail.com, 2026-05-27).
   이 건을 문의하기에 적합한 상대다.

---

# 4. 향료사 연락처

| 향료사 | 담당 | 연락처 | 담당 향료 |
|---|---|---|---|
| Atelier Fragranze Milano (AFM) | 대표 **Luca Maffei** (Ceo · Perfumer) / **Mara Maghini** (Regulatory Affairs Manager) | luca.maffei@ · mara.maghini@atelierfragranzemilano.com · +39 02 4231909 | Olifir Bougie XII, Olifir V4i, Spiritual Wood |
| Chemia Corporation | Regulatory | Regulatory@chemiacorp.com | **84033 Rose Bud** (파운더와 공동 개발) · ~~78296 Coriander (구 소금 향료, 단종)~~ |
| **DreamAir** | Christophe Laudamiel | cl@dreamair.mobi | 뱀부 포레스트 **캔들** 조향 + Bamboom 34 (IFF 시절) |

### AFM 인물 정리 (메일 확인, 2026-09-13)

| 이름 | 역할 | 메일 |
|---|---|---|
| **Luca Maffei** | Ceo · Perfumer (대표 조향사) | luca.maffei@atelierfragranzemilano.com |
| **Mara Maghini** | **Regulatory Affairs Manager** — 서류는 이 사람에게 요청 | mara.maghini@atelierfragranzemilano.com |
| Martina Maffei | (메일 참조자) | martina.maffei@atelierfragranzemilano.com |
| Jasmina Mercuri | Regulatory 지원 | jasmina.mercuri@atelierfragranzemilano.com |
| Marta Brambilla | 물류 | logistica@atelierfragranzemilano.com |
| ~~Federico Ronci~~ | **퇴사함** (2024-05 Mara 공지) | — |

⚠️ **Federica Salvatore (fsalvatore@mrandmrsfragrance.com) 는 AFM 사람이 아니다.**
   Mr&Mrs Fragrance 라는 **다른 회사**의 영업 담당이다 (카 디퓨저 건). 혼동하지 말 것.

---

## 🟢 이미 받은 서류 — 메일함 위치

### `OLIFIR V4I 26830` (부시맨 바디·바 솝)
메일: **`leverden - bushman candle collection -Olifir v4i - L1`** (2026-04-21 전달분)
원본: Mara Maghini → 소영, 2025-10-14 `I: Olifir v4i`

| 첨부 파일 | 내용 |
|---|---|
| `IFRA_51__OLIFIR V4I 26830_VER_1_EN.PDF` | **IFRA 51차 개정판** 인증서 |
| `ALL_OLIFIR V4I 26830_VER_1_EN.PDF / .DOCX / .XLSX` | 알레르겐 선언서 (편집 가능 3종) |
| `SDS_OLIFIR V4I 26830_VER_1_EN.PDF` | SDS |
| `Allergene.xls` | AFM 제공 알레르겐 서식 |

✅ **`Reg 2023/1545` 반영본이다.** Mara의 메일 원문:
> "The list of allergens you provided as an example is still based on the old regulations
> (26 allergens, instead of the current 81), so I am attaching two editable formats …
> updated to Reg 2023_1545."

→ 즉 **81종 기준** 최신본. 우리가 예전에 쓰던 26종 목록은 폐기 대상이다.

📌 같은 메일에서 소영님이 밝힌 사용 계획:
> "launch bar soaps with Olifir V4i … at around **2–3%**"
⚠️ 이는 **계획치**이며 실제 확정 배합률은 `확인 필요`.

### AFM 내부 서류 번호
| 향료 | AFM 문서번호 |
|---|---|
| `SPIRITUAL WOOD 28782` | `AFM-2228782` |
| `OLIFIR BOUGIE XII 26954` | `AFM-2226954` |

### `BAMBOO 34` (뱀부 바디·바 솝)
메일: **`leverden 향료 자료 입니다`** (2025-07-30)
경로: `Cosmocos_Regulatory_Documents/Bamboo_34/BAMBOOM_34 _IFRA_LATEST_6059239_IFRA QRA_17Aug2021_1306271.pdf`
⚠️ **2021년 문서**다. 알레르겐은 26종 기준일 가능성이 높다 → 재요청 대상.

### 기타
- `olifir bougie xii/ spritual wood -test result` (Mara Maghini, 2025-01-10, 첨부 있음)
- `Regulatory Documents` (Federico Ronci, 2023-06-07) — 구버전. IFRA 개정판 확인 필요

---

⚠️ 향료사들은 **전체 성분 목록(full ingredient list)은 영업비밀로 공개하지 않는다.**
   제공 가능한 것은 **IFRA 인증서 · 알레르겐 선언서 · SDS** 세 가지다.
   서류 요청 및 수령은 소영님이 직접 관리한다.

---

# 5. 요청해야 할 것

## 향료사별

**AFM** (Mara Maghini)
- ~~OLIFIR V4i 26830~~ — ✅ **IFRA 51 + 알레르겐(81종) + SDS 수령 완료** (위 §4 참조)
- [ ] OLIFIR BOUGIE XII 26954 (`AFM-2226954`) — 최신 IFRA 개정판 인증서
- [ ] SPIRITUAL WOOD 28782 (`AFM-2228782`) — 최신 IFRA 개정판 인증서
- [ ] 위 2종 알레르겐 선언서 — **`Reg 2023/1545` 81종 기준**으로 명시해서 요청할 것
      (V4i는 이미 81종본을 받았으므로 같은 형식으로 달라고 하면 된다)
- (향 노트는 소영님이 직접 관리 중이므로 별도 요청 불필요)

**Chemia** (Regulatory)
- [ ] 84033 Rose Bud — IFRA 인증서, 알레르겐 선언서
- ~~78296 Coriander~~ — 구 소금 향료로 확인됨. 현행 제품 아님. 요청 불필요

**DreamAir** (Christophe Laudamiel)
- ~~뱀부 포레스트 캔들 향료명~~ — `BAMBOO 28`로 확인됨
- [ ] 캔들 향료 IFRA 인증서 · 알레르겐 선언서 · SDS
- [ ] 탑/미들/베이스 구분 대조 (현재 휘발도 기준 분류)
- [ ] BAMBOOM 34 (6059239) — **2023년 개정 반영 알레르겐 선언서**
- [ ] 최신 IFRA 인증서

## 내부 확인

- [ ] 각 제품의 **실제 향료 배합률** (알레르겐 표시 계산에 필수)
- [ ] 바 솝 2종의 향료 코드
- [ ] Valley of Roses 캔들용 향료 코드

---

# 6. 보관 위치

원문은 Google Drive에 있다.

| 문서 | 파일명 |
|---|---|
| Bamboom 34 알레르겐 선언서 | `BAMBOOM 34 LATEST_6059239_FragrAllerg_17Aug2021_1306252.pdf` |
| Bamboom 34 EU/IFRA 적합 인증 | `BAMBOOM 34 LATEST_6059239_EU_QRA_01Nov2023_3046451.pdf` |
| Bushman 캔들 IFRA | `Cert IFRA50 OLIFIR BOUGIE XII 26954 -EN.PDF` |
| Bushman 바디 IFRA | `Cert IFRA51 OLIFIR V4i 26830 -EN.pdf` |
| Shaman 캔들 IFRA | `Cert IFRA50 SPIRITUAL WOOD 28782 -EN.PDF` |
| Chemia 78296 IFRA | `Chemia 78296 IFRA.pdf` |
| Chemia 78296 SDS | `chemia 78296 sds.pdf` |
| Commiphora wildii (몰약) 알레르겐 선언서 | `KCINPT.Fragrance Allergen Declaration (Commiphora wildii oil) v1.0.pdf` |
| Commiphora wildii 제품 도시에 | `KCINPT.Product Dossier (Commiphora Wildii oil).pdf` |

💡 **몰약(Commiphora wildii) 서류가 별도로 있다.** Myrrh Protection Bar의 원료 근거로 쓸 수 있다.
   힘바족 수급 관련 내용이 도시에에 포함되어 있는지 확인해볼 가치가 있다.
