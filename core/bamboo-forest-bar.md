...
| 항목 | 값 | 출처 |
|---|---|---|
| 탑 | 코리앤더 · 시트러스(리모넨) | `BAMBOOM 34` Essential Oils/Allergen 선언서 (IFF, Doc ID 1325852/1306252) |
| 미들 | 세이지 · 헬리크리섬(이모르텔) | 상동 |
| 베이스 | 엘레미(5-10%, 최대 비중) · 파출리 · 피르 발삼 앱솔루트 · 오크모스 | 상동 |

✅ 2026-09-19 해결 — Google Drive에서 IFF가 발급한 `BAMBOOM 34 LATEST`
Essential Oils and Extracts 선언서 + Fragrance Allergen 선언서를 직접 확인.
공개된 원료는 다음과 같다 (범위%, 향료 완제품 기준):
  - Elemi Oil 5.0-10.0%, Patchouli Oil 1.0-5.0%, Coriander Oil 1.0-5.0%,
    Helychrisum(Immortelle) Oil 1.0-5.0%, Oak Moss Extract 1.0-5.0%,
    Fir Balsam Absolute 1.0-5.0%, Sage Oil 1.0-5.0%
  - 알레르겐 개별 함량: Limonene 4.015%, Linalool 1.747%,
    Oakmoss Extract 1.218%, Geraniol 0.045%, 기타 미량(Citral·Citronellol·
    Eugenol·Farnesol·Benzyl Alcohol)
휘발도 기준으로 위와 같이 탑/미들/베이스에 배치했다 — 향료사(IFF)가
공식적으로 탑/미들/베이스를 구분해 준 문서는 아니고, 공개된 원료명과
일반적인 조향 관례(엘레미·파출리·오크모스=베이스, 세이지·이모르텔=미들,
코리앤더·시트러스=탑)로 재구성한 것이다.

⚠️ 뱀부 포레스트 **캔들**(BAMBOO 28, 탑: 바질·스피어민트 / 미들: 세이지·
코리앤더·이모르텔 / 베이스: 샌달우드·통카·앰버)과 원료가 겹치는 부분
(세이지·코리앤더·이모르텔)이 있으나 **다른 향료(다른 배합)**다 —
바디는 여기에 엘레미·오크모스·피르 발삼이 추가되고 캔들의 바질·통카·
앰버·샌달우드는 없다.
...
| 항목 | 값 | 출처 |
|---|---|---|
| 향료 | **`BAMBOO 34`** (IFF 6059239, `BAMBOOM 34 LATEST`) — 바디·솝용 | `소영`, 알레르겐 선언서 |
| Oakmoss 함량 | 1.218% | 알레르겐 선언서 (Doc ID 1306252) |
...
| # | 항목 | 필요한 것 |
|---|---|---|
| 1 | ~~향 노트 전체 + 탑/미들/베이스~~ | ✅ 해결 (2026-09-19) — 위 §2 참조. IFF 선언서 기반으로 재구성 |
| 2 | Oakmoss 표시 의무 | EU 규정 2023/1545 대응 확인 (함량 1.218%로 확인됨 — 표시 의무 여부만 남음) |
