...
| 항목 | 값 | 출처 |
|---|---|---|
| 탑 (`OLIFIR V4i` 향료) | 시트러스 — Citrus Aurantium Peel Oil · 리모넨 · 피넨 | `ALL_OLIFIR V4I 26830` 알레르겐 원료표 |
| 미들 (`OLIFIR V4i` 향료) | 제라늄·플로럴 — 제라니올 · 제라닐 아세테이트 · 리날룰 | 상동 |
| 베이스 (`OLIFIR V4i` 향료) | 시더우드(Juniperus Virginiana) · 파출리(Pogostemon Cablin) · 바닐린 · 쿠마린 · 머스크(헥사메틸인다노피란·테트라메틸아세틸옥타하이드로나프탈렌) | 상동 |

📌 **몰약(Commiphora Wildii Oil)은 `OLIFIR V4i` 향료 성분표엔 없다.** 전성분에
`Fragrance`와 별도 항목으로 기재되어 있으므로(코보큐앤에이 원본 참조),
향료사가 조향한 향과 별개로 **공장에서 직접 추가하는 원료**다. 노트 구조상으로는
몰약도 무거운 수지향이라 베이스에 가깝지만, 향료 배합표에 포함된 원료가 아니므로
위 베이스 노트와 구분해서 "몰약(별도 첨가)"으로 표기한다.

| 향료 | `OLIFIR V4i` (26830) — **바디 · 바 솝용** | AFM, `소영` (2026-09-13) |
| **몰약 학명** | **`Commiphora Wildii Oil`** | `소영` (2026-09-13 확정) |
| 순 중량 | **250 g** | 코보큐앤에이 · `소영` (2026-09-13) |

### 몰약 종 확정

| 표기 | 상태 |
|---|---|
| ~~`Commiphora myrrha`~~ | ❌ 고전적 몰약(소말리아·에티오피아산). **다른 종이다** |
| ~~`Myrrh Oil`~~ | ❌ 공장 표기. 일반명이라 종을 특정하지 못한다 |
| **`Commiphora Wildii Oil`** | ✅ **확정.** 나미비아 몰약(오뭄비리) |

📌 **힘바 서사가 성립하려면 `Commiphora wildii` 여야 한다.** 힘바 여성들이
   채취하는 오뭄비리가 바로 이 종이다. `myrrha`로 적으면 원료 이야기와
   전성분이 서로 다른 것을 가리키게 된다.
...
✅ **노트 목록 해결 (2026-09-19)** — `ALL_OLIFIR V4I 26830` 알레르겐 원료표의
   원료명과 함량으로 탑/미들/베이스를 구성함(위 표 참조). 전체 조합 향료의
   퍼센트 전부가 공개된 건 아니고(알레르겐 대상 원료만 공개), 서열은 원료
   성격(휘발도)에 따라 합리적으로 배치한 것 — 향료사(AFM) 공식 노트
   구분 문서는 아니다.
