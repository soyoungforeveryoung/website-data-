...
| 단계 | 값 | 한글 |
|---|---|---|
| 탑 | Basil, Spearmint | 바질 · 스피어민트 |
| 미들 | Sage, Coriander, Immortelle | 세이지 · 코리앤더 · 임모르텔 |
| 베이스 | Sandalwood, Tonka, Amber | 샌달우드 · 통카 · 앰버 |

출처: **휘발도 기준 분류 + `소영` 확정** (2026-09-13). 원 자료는 평면 목록이며 단계 구분이 없었다.
부시맨 캔들·샤먼·밸리 오브 로지즈는 원문에 단계가 명시되어 있으나 이 제품만 없다.

**근거**
- 탑 — 바질·스피어민트는 휘발성 허브. 스피어민트는 거의 즉시 날아간다
- 미들 — 임모르텔은 전형적인 하트 노트
- 베이스 — 샌달우드·통카·앰버는 셋 다 고정제. 이견의 여지가 없다

✅ **코리앤더는 미들로 확정** (`소영`, 2026-09-13).
   리날로올 함량이 높아 탑으로 분류하는 조향사도 있으나, 이 제품에서는 미들이다.
   세이지도 미들에 둔다.

→ 향료 노트 문서를 받으면 이 분류와 대조할 것. 다르면 문서를 따른다.
...
| 조향 회사 | **DreamAir** — 로다미엘 본인의 회사 | `소영` (2026-09-13) |
| 향료 | **`BAMBOO 28`** — 캔들용 | `소영` (2026-09-13) |

**뱀부 포레스트는 향료가 둘로 갈린다** (`소영`, 2026-09-13).

| 라인 | 향료 |
|---|---|
| **캔들** | `BAMBOO 28` |
| 바디 · 바 솝 | `BAMBOO 34` (IFF 6059239) |

태우는 것과 몸에 쓰는 것은 규제 체계가 다르다. 서로 바꿔 표기하지 않는다.

🔴 이전에 `Chemia 78296 CORIANDER FRAGRANCE`로 기록했던 것은 무효다.
   그 향료는 **예전 소금(배스 솔트) 제품용**이며 단종됐다.
   코리앤더가 이 제품의 향 노트라서 이름이 겹쳐 잘못 연결한 것이다.

- 로다미엘은 원래 **IFF 소속**이었다가 독립해 DreamAir를 차렸다 (`소영`)
- 그래서 **바디 라인 향료가 IFF**(`BAMBOOM 34`, IFF 6059239)인 것과 연결된다.
  IFF 시절에 만든 향으로 보인다

⚠️ `BAMBOO 28` 서류(IFRA·알레르겐·SDS)는 **DreamAir**에 요청한다 (cl@dreamair.mobi).
...
| # | 항목 | 필요한 것 |
|---|---|---|
| 1 | ~~향 노트 탑/미들/베이스 구분~~ | ✅ 확정. DreamAir 노트 문서 수령 시 대조 |
